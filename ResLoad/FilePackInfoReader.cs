...
namespace ResLoad
{
	...
	{
		public static void Read(ref Dictionary<string, ResPackageInfo> PackedResInfosDic, ref Dictionary<string, PackedFileInfo> filesPackInfoDic, string packedResPath)
		{
			FileStream fs = new FileStream(packedResPath, FileMode.Open);

			ResPackageInfo tempResInfo = new ResPackageInfo ();

			//1.资源包信息区域
			//1.1资源包版本（int)
			byte[] resVersionData = new byte[4];
			fs.Read (resVersionData, 0, 4);
			tempResInfo.Version = MyBinaryReader.ReadInt (resVersionData);
			resVersionData = null;
			...
			tempResInfo.TypeName = MyBinaryReader.ReadString (resTypeNameData);
			resTypeNameData = null;
			//1.4读取资源包总共字节大小(uint)
			byte[] resTotalSizeData = new byte[4];
			fs.Read (resTotalSizeData, 0, 4);
			tempResInfo.TotalSize = MyBinaryReader.ReadUint(resTotalSizeData);
			resTotalSizeData = null;
			//获取资源包名称
			FileInfo resInfo = new FileInfo(packedResPath);
			string resName = resInfo.Name.Split ('.')[0];
			PackedResInfosDic.Add (resName, tempResInfo);
